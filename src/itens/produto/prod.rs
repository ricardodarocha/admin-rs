...
pub async fn newprod(mut db: Connection<DbMeuBanco>, product: Json<Product>) -> Result<Json<Product>, status::Custom<String>> {
    sqlx::query_as::<_, Product>("INSERT INTO produto (name, price, tipo) values (?, ?, 'produto') returning *")
        .bind(&product.name)
        .bind(product.price)
        .fetch_one(&mut **db)
        .await
...
pub async fn getallprod(mut db: Connection<DbMeuBanco>) -> Result<Json<Vec<Product>>, status::Custom<String>> {
    sqlx::query_as::<_, Product>("SELECT * FROM produto where tipo = 'produto'")
        .fetch_all(&mut **db)
        .await
        .map(Json)
        .map_err(|e| status::Custom(Status::InternalServerError, e.to_string()))
...
pub async fn getprod(mut db: Connection<DbMeuBanco>, id: i32) -> Result<Json<Product>, status::Custom<String>> {
    sqlx::query_as::<_, Product>("SELECT * FROM produto where id = ?/*id*/ and tipo = 'produto'")
        .bind(id)
        .fetch_one(&mut **db)
        .await
        .map(Json)
