...
#[get("/recuperar")]
async fn recover(data: web::Data<AppState>) -> impl Responder {
    let tmpl = data.render.get_template("auth/recover.html").unwrap();
    let rendered = tmpl.render(context! {title => "Recuperar"}).unwrap();
